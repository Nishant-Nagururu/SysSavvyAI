...
const RunCard = ({ run, highlight = false }) => {
  ...
  return (
    ...
    <Card sx={{ mb: 2, borderRadius: 2, boxShadow: 1, border: borderStyle, borderColor }}>
        ...
        <CardContent sx={{ pt: 1 }}>
              ...
              {costEstimates.map((item, index) => (
                ...
                >
                  <Typography variant="body2">
                    Proposed Monthly Cost: ${Number(item['proposed-monthly-cost']).toFixed(2)}
                  </Typography>
                </Box>
              ))}
            </Box>
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 1 }}>
            ...
            {actions['is-discardable'] && (
              <Button size="small" variant="outlined" color="error" onClick={handleDiscard}>
                Discard Run
              </Button>
            )}
          </Box>
      ...
      <CardActions>
          {actions['is-confirmable'] && <Button size="small">Approve Run</Button>}
          {actions['is-discardable'] && (
            <Button size="small" color="error">Discard Run</Button>
          )}
          <Button size="small" onClick={openModal}>More Details</Button>
        </CardActions>
    </Card>
  ...
  <DialogContent sx={{ height: 'calc(100% - 64px)' }}>
    <Grid container spacing={2} sx={{ height: '100%' }}>
      ...
      <Grid item xs={6} sx={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
        <Typography variant="h6" sx={{ mb: 1 }}>Terraform Files</Typography>
        <Box
          sx={{
            flexGrow: 1,
            width: '100%',
...
<Grid item xs={6} sx={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
  ...
  >
    <Typography variant="h6" sx={{ mb: 1 }}>Plan Log</Typography>
    <Box sx={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: 'monospace' }}>
      {planLog}
    </Box>
  </Box>
  ...
  >
    <Typography variant="h6" sx={{ mb: 1 }}>Apply Log</Typography>
    <Box sx={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: 'monospace' }}>
      {applyLog}
    </Box>
  </Box>
